...
class Visualizer:
    ...
    def prepare(self, chart_data):
        ...
        ax = self.axes[0].twinx()       # 2개의 x축 사용: x축을 volume(좌), stock_price(우) 2개로 만든다.

        # horizen-stack이므로, 인덱스 배열(x.reshape(-1,1)_ + 차트 데이터 일부인 OHLC(np.array(chart_data)[:, 1:-1])
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None, action_list=None, actions=None, num_stocks=None,
        ...
        x = np.arange(len(actions))     # 모든 차트가 공유할 x축 데이터 (actions의 길이에 맞춘 인덱스 배열)
        # 이 x축 데이터는 모든 차트가 공유한다. 이때 actions, num_stocks, outvals, exps, pvs의 모든 크기가 같기 때문에
        # 이들 중 하나인 actions의 크기만큼 배열을 생성하여 x축으로 사용한다.
        # TODO --> 이때 이들이 같은 이유는 일마다 거래를 하기 때문에 같은 것으로 보인다. --> 체크 필수!

        actions = np.array(actions)     # 에이전트의 행동 배열 --> actions는 리스트이고 Matplotlib는 배열을 입력으로 받기 때문에 배열로 변환한다.
        outvals = np.array(outvals)     # 정책 신경망의 출력 배열 --> outvals는 리스트이고 Matplotlib는 배열을 입력으로 받기 때문에 배열로 변환한다.
        pvs_base = np.zeros(len(actions)) + initial_balance     # 초기 자본금 배열 (초기 자본금 직선): 포트폴리오 가치 차트에서 초기 자본금에 직선을 그어서,
        # 포트폴리오 가치와 초기 자본금을 쉽게 비교할 수 있도록 배열(pvs_base)로 준비하여 직선이 되도록 한다..


        # 차트 2. 에이전트 상태 (행동, 보유 주식 수) : 에이전트가 수행한 행동을 배경의 색으로 표시하고, 부유 주식 수를 라인 차트로 그린다.
        ...
        # action이 Explore가 아닌 Exploit인 경우
        ...
        for action, style in zip(action_list, styles):
            # 정책 신경망의 출력을 빨간색 또는 파란색 점으로 그리기
    ...
    # Figure 초기화
    def clear(self, xlim):      # (입력으로 받는 xlim은 모든 차트의 x축 값 범위를 설정해 줄 튜플이다.)
